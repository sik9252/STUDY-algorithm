...
- 데이터를 제한적으로 접근할 수 있는 구조 ⇒ 한쪽 끝에서만 자료를 넣거나 꺼낼수 있는 구조
- 가장 나중에 쌓은 데이터를 가장 먼저 빼낼 수 있는 구조(LIFO or FILO) ⇒ 큐와 차이점
...
즉, 함수 호출시 프로세스 실행 구조를 스택과 비교해서 이해하는것이 필요하다.

### 자료 구조 스택의 장/단점

- [장점] 구조가 단순, 구현이 쉬움
- [장점] 데이터 저장/읽기 속도가 빠름
- [단점] 데이터 최대 개수를 미리 정해야 함(파이썬 재귀는 1000번까지만 호출 가능)
- [단점] 저장 공간의 낭비가 발생할 수 있음

스택은 단순하고 빠른 성능을 위해 사용됨. 그러므로 보통 배열 구조를 이용해서 구현하는것이 일반적임
...
1. 리스트 변수로 스택을 다루는 push, pop 기능 구현해보기
